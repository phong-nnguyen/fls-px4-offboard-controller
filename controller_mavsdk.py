...
class Controller:
    ...
    async def connect(self):
        if self.sim or self.router:
            ...
            connection_string = f"serial://{self.device}:{self.baudrate}"
        ...
        async for state in self.master.core.connection_state():
            if state.is_connected:
                ...
                break
        self.connected = True
        info = await self.master.info.get_version()
        self.logger.info(
            f"Connection from system (system {info.flight_sw_major} component {info.flight_sw_minor})")
